...
const ProfileScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    fetch(`https://darkvilla.onrender.com/api/post/${profile.userId}`, {
      headers: {
        Authorization: `Bearer ${token.access_token}`,
      },
    })
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {activeTab === "Posts" ? (
          posts.length === 0 ? (
            ...
            >
              {profile.userId === user._id ? "You don't have any post." : "User don't have any post."}
            </Text>
          ) : (
            <PostContent posts={posts} navigation={navigation} />
          )
        ) : videos.length === 0 ? (
          <Text
            style={{
